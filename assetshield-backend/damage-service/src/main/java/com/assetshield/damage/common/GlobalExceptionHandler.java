...
public class GlobalExceptionHandler {
    ...
    /**
     * Final guard for races that slip past the explicit pre-checks: the unique
     * indexes are translated to their domain 409s instead of surfacing as 500.
     */
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> handleIntegrity(DataIntegrityViolationException e) {
        ...
        if (detail.contains("ux_dmgphoto_report_hash")) {
            return ResponseEntity.status(ErrorCode.DUPLICATE_PHOTO_HASH.status())
                    .body(ApiResponse.error(ErrorCode.DUPLICATE_PHOTO_HASH,
                            "An identical photo already exists on this damage report", null));
        }
        if (detail.contains("ux_pair_unique")) {
            return ResponseEntity.status(ErrorCode.PAIR_EXISTS.status())
                    .body(ApiResponse.error(ErrorCode.PAIR_EXISTS,
                            "This photo is already paired with that asset", null));
        }
