...
public class GlobalExceptionHandler {
    ...
    /**
     * Final guard for races that slip past the explicit pre-checks: the
     * partial unique indexes are translated to their domain 409s instead of
     * surfacing as a 500.
     */
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> handleIntegrity(DataIntegrityViolationException e) {
        ...
        if (detail.contains("ux_assets_property_hash")) {
            return ResponseEntity.status(ErrorCode.DUPLICATE_ASSET_HASH.status())
                    .body(ApiResponse.error(ErrorCode.DUPLICATE_ASSET_HASH,
                            "An identical photo already exists on this property", null));
        }
        if (detail.contains("ux_invite_pending")) {
            return ResponseEntity.status(ErrorCode.DUPLICATE_PENDING_INVITE.status())
                    .body(ApiResponse.error(ErrorCode.DUPLICATE_PENDING_INVITE,
                            "A pending invitation already exists for this phone number", null));
        }
        if (detail.contains("ux_membership_active")) {
            return ResponseEntity.status(ErrorCode.ALREADY_MEMBER.status())
                    .body(ApiResponse.error(ErrorCode.ALREADY_MEMBER,
                            "This user is already an active member of the property", null));
        }
