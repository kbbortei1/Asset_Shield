...
/**
 * Streams local-provider user images (avatars) while their token is live —
 * possession of an unexpired token IS the authorization, exactly like a signed
 * URL. With the supabase provider no tokens are issued (reads use presigned S3
 * URLs), so every request here 404s.
 */
@RestController
@RequestMapping("/api/v1/public/user-files")
public class PublicUserFileController {

    private final DownloadTokenStore tokenStore;
    private final StorageProvider storageProvider;

    public PublicUserFileController(DownloadTokenStore tokenStore, StorageProvider storageProvider) {
